<?php
/* PPK JoyAsset DEMO based Bytom Blockchain */
...
<h2 align="right">返回<a href="./">主页</a></h2>
</div>
<div id="yourinfo">
<!--
...
<p>资产属性定义：</p>
<p align='center'>
<textarea id="asset_setting_json" rows=25 cols=50 onchange="updateTransData();"  >
{
  "@context": [
        "https://schema.org/",
  ...
  "name": "Test1",
  "avtar": "http://btmdemo.ppkpub.org/asset/image/asset.png",  
...
<p>转账GAS费用：<input type="text" name="asset_trans_fee_btm" id="asset_trans_fee_btm" value="<?php echo TX_GAS_AMOUNT_mBTM/1000; ?>" size=10 readonly="true" style="background:#CCCCCC"> BTM</p>
...
function sendBtmTX() {
  if(document.getElementById('asset_setting_json').value.length == 0 ){
    alert("请输入有效的资产定义！");
    return false;
  }

  if(document.getElementById('asset_trans_fee_btm').value.length == 0 ){
  ...
  document.getElementById("send_trans_btn").value="正在自动注册资产标识,请稍候...";
...
function updateTransData(){
  var asset_trans_fee_btm = <?php echo TX_GAS_AMOUNT_mBTM/1000; ?>;
  

  var str_setting=document.getElementById('asset_setting_json').value;
  ...
  //var btm_uri='bytom:'+document.getElementById('guess_contract_uri').value+'?value='+asset_trans_fee_btm+'&data='+iot_trans_data_hex;
...
function resetAll(){
  document.getElementById('asset_trans_fee_btm').value=<?php echo TX_GAS_AMOUNT_mBTM/1000; ?>;
